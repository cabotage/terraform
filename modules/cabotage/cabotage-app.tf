...
locals {
  cabotage_app_config_data = merge({
    ...
    CABOTAGE_TENANT_POSTGRES_BACKUPS_ENABLED                       = var.tenant_postgres_backup_enabled ? "True" : "False"
    CABOTAGE_TENANT_POSTGRES_BACKUP_BUCKET                         = local.use_s3 ? var.s3_storage.postgres_backup_bucket : "cabotage-postgres-backups"
    CABOTAGE_TENANT_POSTGRES_BACKUP_IRSA_ROLE_ARN                  = local.use_s3 ? var.s3_storage.tenant_postgres_backup_role_arn : ""
    CABOTAGE_TENANT_POSTGRES_BACKUP_PATH_PREFIX                    = var.tenant_postgres_backup_path_prefix
    CABOTAGE_TENANT_POSTGRES_BACKUP_PLUGIN_NAME                    = var.tenant_postgres_backup_plugin_name
    CABOTAGE_TENANT_POSTGRES_BACKUP_PROVIDER                       = local.use_s3 ? "s3" : "rustfs"
    CABOTAGE_TENANT_POSTGRES_BACKUP_RETENTION_POLICY               = var.tenant_postgres_backup_retention_policy
    CABOTAGE_TENANT_POSTGRES_BACKUP_RUSTFS_CA_SECRET_NAME          = local.use_s3 ? "" : var.tenant_postgres_backup_rustfs_ca_secret_name
    CABOTAGE_TENANT_POSTGRES_BACKUP_RUSTFS_ENDPOINT                = local.use_s3 ? "" : var.tenant_postgres_backup_rustfs_endpoint
    CABOTAGE_TENANT_POSTGRES_BACKUP_RUSTFS_SECRET_NAME             = local.use_s3 ? "" : var.tenant_postgres_backup_rustfs_secret_name
    CABOTAGE_TENANT_POSTGRES_BACKUP_RUSTFS_SOURCE_SECRET_NAME      = local.use_s3 ? "" : var.tenant_postgres_backup_rustfs_source_secret_name
    CABOTAGE_TENANT_POSTGRES_BACKUP_RUSTFS_SOURCE_SECRET_NAMESPACE = local.use_s3 ? "" : var.tenant_postgres_backup_rustfs_source_secret_namespace
    CABOTAGE_TENANT_POSTGRES_BACKUP_SCHEDULE                       = var.tenant_postgres_backup_schedule
    CABOTAGE_TENANT_POSTGRES_BACKUP_SERVICE_ACCOUNT_NAME           = var.tenant_postgres_backup_service_account_name
